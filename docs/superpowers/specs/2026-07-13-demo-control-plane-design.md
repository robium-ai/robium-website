...
registry of available demos. It fills the role Cloud Run's control plane
...
(that stays inside each container, unchanged) — the control plane manages
lifecycle and then steps out of the data path.

## Scope (this spec = LOCAL only)

Per the brainstorm: build the control plane for the **local (Docker)**
backend first, behind a clean API. The **cloud model is deferred** — a
later spec decides how much of Cloud Run's autoscaling the control plane
...
3. **Lifecycle-only + direct connect** — the control plane starts/stops/
   ...
   unchanged). The control plane is never in the data path. Budget is
...
browser (robium.org/demos/<demo>)
   ...
   │  2. POST /api/instances {demo}     → control plane spawns container,
   │                                       returns { id, host, session }
   │  3. …talks DIRECTLY to that sim's gateway at `host` for
   │      status / logs / pty / fs / foxglove tunnel (unchanged) …
   │  4. DELETE /api/instances/:id      → control plane stops the container
   ▼
control plane (Node/TS, host process, port 8080)   ← the new tier
...
- **Registry** (`control-plane/demos.json`): declares each demo. Fields:
...
- **Shared types** (`control-plane/types.ts`, imported by the frontend):
  `Demo`, `Instance`, request/response shapes — one source of truth.

## Instance readiness

The control plane reports only *container* lifecycle (created/running/exited)
...
as today). Clean seam: control plane = "is the box up?", gateway = "is the
robot ready?".

## Frontend changes

- New client `src/lib/controlPlane.ts` (typed, shares `control-plane/types`):
...
- `Workspace.tsx`: **Start** → `createInstance('nav-trial', session)` →
  ...
  **Stop** → `deleteInstance(id)` (real stop, everywhere — no more
  local-special-casing; the container genuinely goes away and the control
  plane can spawn a fresh one on the next Start).
- The dev `?host=`/switcher stays as an escape hatch (talk to a hand-started
  container directly, bypassing the control plane) but the default path is
  the control plane.
- Control-plane base URL: `http://localhost:8080` in dev; the deployed value
  is set when the cloud spec lands (env/build-time).

## Config / running

- `control-plane/` package (its own package.json, TS, `npm run dev` /
  `npm start`). A root `make control-plane` alias just runs it — the service
...
2. **Orphan cleanup** — a session that never calls DELETE (tab crash) leaves
   a container; the control plane reaps instances past `sessionSeconds`
...
4. **Same-origin/CORS** — the browser now calls two backends (control plane
...
for a public control plane, per-user quotas, a management dashboard UI,
